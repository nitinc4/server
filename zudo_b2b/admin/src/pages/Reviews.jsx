...
import { Star, Trash2, MessageSquare, User, Package, Image as ImageIcon, Video, Loader2, Calendar, Quote } from 'lucide-react';
...
const Reviews = () => {
  ...
  if (loading) return (
    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', height: '60vh', gap: '16px' }}>
      ...
      <div style={{ color: '#94a3b8', fontWeight: 500 }}>Fetching Feedback...</div>
    </div>
  );
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gap: '24px' }}>
        {reviews.map((review) => (
          <div key={review._id} className="glass-card" style={{ padding: '24px', borderRadius: '24px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: '11px', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{review.userId?.email}</div>
                </div>
              </div>
              <button 
            ...
            <div style={{ padding: '12px 16px', background: 'rgba(255,255,255,0.02)', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <p style={{ fontSize: '14px', color: '#cbd5e1', lineHeight: '1.6', margin: 0, paddingLeft: '16px', fontStyle: 'italic' }}>
                {review.comment}
              </p>
            </div>
                  ...
                  <div key={idx} style={{ position: 'relative', width: '80px', height: '80px', borderRadius: '12px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.05)' }}>
                    {item.type === 'image' ? (
                      ...
                      <div style={{ width: '100%', height: '100%', background: '#0f172a', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <Video size={20} style={{ color: 'white', opacity: 0.5 }} />
                      </div>
                    )}
                    <a href={getFullUrl(item.url)} target="_blank" rel="noreferrer" style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.4)', opacity: 0, transition: '0.3s', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '10px', fontWeight: 700, textDecoration: 'none' }} className="media-overlay">VIEW</a>
                  </div>
                ))}
              </div>
            )}
            ...
            <div style={{ marginTop: 'auto', paddingTop: '16px', borderTop: '1px solid rgba(255,255,255,0.03)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '10px', color: '#475569', fontWeight: 700, textTransform: 'uppercase' }}>
        ...
        {reviews.length === 0 && (
          <div style={{ gridColumn: '1/-1', padding: '80px', textAlign: 'center', color: '#475569' }}>
