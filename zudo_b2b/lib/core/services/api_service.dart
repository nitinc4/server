...
class ApiService {
  ...
  static Future<void> applyCommissions(String token) async {
    final response = await http.post(
      Uri.parse('${ApiConstants.baseUrl}/products/apply-commissions'),
      headers: _getHeaders(token),
    );
    if (response.statusCode != 200) {
      throw Exception('Failed to apply commissions: ${response.body}');
    }
  }
  ...
  static Future<List<dynamic>> fetchDeliverySlots(String token) async {
    final response = await http.get(
      Uri.parse('${ApiConstants.baseUrl}/orders/slots'),
      headers: _getHeaders(token),
    );
    return List<dynamic>.from(_processResponse(response, 'Failed to fetch delivery slots'));
  }
