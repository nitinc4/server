...
class ProductVariant {
  ...
  factory ProductVariant.fromJson(Map<String, dynamic> json) {
    ...
    return ProductVariant(
      ...
      gstPercent: double.tryParse(json['gstPercent']?.toString() ?? json['GST']?.toString() ?? '') ?? (json['gstPercent'] is num ? (json['gstPercent'] as num).toDouble() : (json['GST'] is num ? (json['GST'] as num).toDouble() : 0.0)),
      priceTiers: tiers,
    );
  }
}
...
class Product {
  ...
  factory Product.fromJson(Map<String, dynamic> json) {
    ...
    if (variantsList == null || variantsList.isEmpty) {
      variantsList = json['b2b'] as List? ?? json['b2c'] as List? ?? [];
    }
  ...
  double getPriceForQuantity(int quantity, bool isB2B, [String? variantSize]) {
    double basePrice = getBasePriceForQuantity(quantity, isB2B, variantSize);
    if (!isB2B) return basePrice;
    double activeGst = getGstPercent(variantSize);
    return basePrice + (basePrice * activeGst / 100);
  }

  double getBasePriceForQuantity(int quantity, bool isB2B, [String? variantSize]) {
  ...
  double getGstPercent([String? variantSize]) {
    return gstPercent;
  }
